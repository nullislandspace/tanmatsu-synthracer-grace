...
| Barrel roll (corkscrew) | ✅ done 2026-05-22. While fully banked one way (\|bank\| ≥ `GAME_BARREL_TRIGGER_BANK` 0.9) with that side held, also press the opposite side → the ship rolls a full turn and snaps to the opposite max bank. Re-arms only after both steer buttons are released together. Detected in `game_step` from the raw `input_steer_held()` pair (not the collapsed `input_steering()`). Visual-only flourish (`game_state.roll_spin`) + a forced bank flip; collision is unaffected. **No** invulnerability frame (cosmetic + repositioning). |
