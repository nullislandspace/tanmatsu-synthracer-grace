...
| 9.3 | Checkpoint pickup | ✅ done 2026-05-19 — black-and-white icosahedron pickup (`objects/checkpoint.c`, a soccer-ball reskin of the speed booster); scheduled once per stage from stage 5 via the owed-pickup API; collecting one plays a procedural gong (`sfx/sfx_gong.c`) and snapshots the whole run state (a struct copy of `world_state_t` + `game_state_t` — obstacle pool, area, RNG, wall cursors, ship, scores, stats, sun, inventory); a head-on crash with a checkpoint (and no shield) restores that snapshot and opens the pause-like **`APP_STATE_CHECKPOINT_REDO`** dialog ("Re-Do from checkpoint" + Churchill quote, scene frozen behind, music keeps playing); space resumes — checkpoint spent, shield window granted. HUD: a black/white checkerboard. Built **ungated**. See the 2026-05-19 Phase 9.3 decisions-log entries. |
