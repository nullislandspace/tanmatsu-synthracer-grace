...
- [ ] Move projection/camera from `render.c` to the engine. **Invert** `render_submit_obstacles` / `render_shadows`: engine exposes a shadow pass over a `se_shadow_caster_t[]` span (array, not callback); the world-aware iteration moves game-side. Engine drops `#include "world.h"`.
- [ ] Split `magicnumbers.h`: engine consts (`RENDER_*`, scene, near-clip) → `se_config.h` `#ifndef` defaults; `GAME_*` tuning stays game-side; verify `RENDER_HORIZON_Y` is game-overridable.
- [ ] Build + verify + **FPS/map check** (most perf-sensitive move).
